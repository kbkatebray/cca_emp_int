...
np_rc_f_alt1 <- read.csv("../scored_data/non-para_rc_F_alt1.csv", header=TRUE, stringsAsFactors = FALSE)
np_rc_f_alt2 <- read.csv("../scored_data/non-para_rc_F_alt2.csv", header=TRUE, stringsAsFactors = FALSE)
...
apa_table(
  list(`Sex` = desctable[1:2,2:3], `Age` = desctable[3,2:3],`Est. FSIQ` = desctable[4,2:3], `Parent Education` = desctable[5:12,2:3], `Neighborhood advantage` = desctable[13:17,2:3], `Race` = desctable[18:21,2:3], `Ethnicity` = desctable[22:28,2:3] ),
          caption = "Descriptive statistics characterizing the sample used for the CCA analysis",
          note = "Est. FSIQ = Estimated full scale IQ, Neighborhood advantage is listed from least advantage to most",
          align = c("l",rep("c", 2)),
...
cronemp <- papaja::printnum(rbind(c1$total[2], c2$total[2], c3$total[2], c4$total[2], c5$total[2]), gt1 = FALSE) # the printnum plus gt1 = FALSE argument removes leading zeros
croncdi <- papaja::printnum(rbind(c6$total[2], c7$total[2], c8$total[2], c9$total[2]), gt1 = FALSE)
cronscas <- papaja::printnum(rbind(c10$total[2], c11$total[2], c12$total[2], c13$total[2], c14$total[2], c15$total[2]), gt1 = FALSE)
...
row.names(cdi_desc) <- c("Neg. mood/phys. symptoms","Negative self-esteem","Ineffectiveness","Interpersonal problems")
...
apa_table(cbind(ccatable[,1], papaja::printnum(ccatable[,2:4], gt1 = FALSE), ccatable[,5:7], papaja::printp(ccatable[,8]), papaja::printp(np_rc_f$np_f), papaja::printp(ccatable[,9]), papaja::printp(np_rc_f$np_rc)),
          ...
          align = c("l",rep("c", 10) ),
          col.names = c("Dimension",	"Rc",	"Rc2",	"Wilk’s $\\Lambda$",	"Multiple $F$",	"df1",	"df2",	"Para. $p$ ($F$)","Non-para. $p$ ($F$)",	"Para. $p$ (Rc)",	"Non-para. $p$ (Rc)"),
          font_size = "small"
)
```
...
apa_table(cbind(xcoeftable[,1], papaja::printnum(xcoeftable[,2:3], gt1 = FALSE), papaja::printp(xcoeftable[,4]), papaja::printp(NPX$x)),
          caption = "Coefficients for X variables, first function.",
          note = "* Survive correction for multiple comparisons (Bonferroni).",
          align = c("l",rep("c", 5) ),
          col.names = c("X Variables", "Standardized X canonical coefficients",	"Structure coefficient (rs)",	"Squared structure",	"Para. $p$ (rs)", "Non-para. $p$ (rs)"),
          font_size = "small"
)
```
...
apa_table(cbind(ycoeftable[,1], papaja::printnum(ycoeftable[,2:3], gt1 = FALSE),papaja::printp(ycoeftable[,4]), papaja::printp(NPY$x)),
          caption = "Coefficients for Y variables, first function.",
          note = "* Survive correction for multiple comparisons (Bonferroni).",
          align = c("l",rep("c", 5) ),
          col.names = c("Y Variables", "Standardized X canonical coefficients",	"Structure coefficient (rs)",	"Squared structure",	"Para. $p$ (rs)", "Non-para. $p$ (rs)"),
          font_size = "small"
)
...
row.names(t_freq) <- c("Average (40-60)","High Average (60-65)", "Elevated (65-70)", "Very elevated (70+)", "Average (40-60).", "Elevated (60-65) - top 16% of population", "(65-70) - top 6% of population", "(70+) - top 2% of population")


apa_table(
  list(`CDI` = t_freq[1:4,2:3], `SCAS` = t_freq[5:8,2:3] ),
          caption = "T-scores from the CDI and SCAS measures",
          note = "T-scores have a mean of 50 and a standard deviation of 10. There were 25 children (20%) scoring higher than average depressive symptoms in the sample.  For anxiety symptoms, there were only 6 children (5%) scoring higher than average. The t-scores suggest 16% of the population in this age group might be expected to score higher than average. (For CDI see Kovacs, 2010; for SCAS see https://www.scaswebsite.com/index.php?p=1_9).",
          align = c("l",rep("c", 2)),
...
apa_table(cbind(alt1_ccatable[,1], papaja::printnum(alt1_ccatable[,2:4], gt1 = FALSE), alt1_ccatable[,5:7], papaja::printp(alt1_ccatable[,8]), papaja::printp(np_rc_f_alt1$np_f), papaja::printp(alt1_ccatable[,9]), papaja::printp(np_rc_f_alt1$np_rc)),
          caption = "Pertinent values from the CCA analysis for each of the five canonical functions created from alternate model one",
          note = "* = <0.05, bold text = both parametric and non-parametric p-value significant. Rc = Canonical Correlation, Rc2 = Canonical Correlation Squared. Although the first and second functions were both significant, only the first function was interpreted as this was consistent across all three models (in addition the main associations within the second function appear to be between the cognitive empathy task and IQ).",
          align = c("l",rep("c", 10) ),
          col.names = c("Dimension",	"Rc",	"Rc2",	"Wilk’s $\\Lambda$",	"Multiple $F$",	"df1",	"df2",	"Para. $p$ ($F$)","Non-para. $p$ ($F$)",	"Para. $p$ (Rc)",	"Non-para. $p$ (Rc)"),
          font_size = "small"
)
...
row.names(alt1_xcoeftable) <- c("Affective Sharing","Cognitive Empathy","Empathic Concern","Empathic Distress","Silent Films")
alt1_xcoeftable$X <- NULL

row.names(alt1_ycoeftable) <- c("Negative mood/physical symptoms","Negative self-esteem","Ineffectiveness","Interpersonal problems",
                           "Separation anxiety","Social phobia","Physical injury fears","Generalized anxiety","Obsessive compulsive", "Panic/agoraphobia","Sex","IQ","Parent Education","Neighborhood Advantage")
...
apa_table(cbind(alt1_xcoeftable[,1], papaja::printnum(alt1_xcoeftable[,2:3], gt1 = FALSE), papaja::printp(alt1_xcoeftable[,4]), papaja::printp(alt1_NPX$x)),
          caption = "Coefficients for X variables, first function.",
          note = "* Survive correction for multiple comparisons (Bonferroni).",
          align = c("l",rep("c", 5) ),
          col.names = c("X Variables", "Standardized X canonical coefficients",	"Structure coefficient (rs)",	"Squared structure",	"Para. $p$ (rs)", "Non-para. $p$ (rs)"),
...
apa_table(cbind(alt1_ycoeftable[,1], papaja::printnum(alt1_ycoeftable[,2:3], gt1 = FALSE),papaja::printp(alt1_ycoeftable[,4]), papaja::printp(alt1_NPY$x)),
          caption = "Coefficients for Y variables, first function.",
          note = "* Survive correction for multiple comparisons (Bonferroni).",
          align = c("l",rep("c", 5) ),
          col.names = c("Y Variables", "Standardized X canonical coefficients",	"Structure coefficient (rs)",	"Squared structure",	"Para. $p$ (rs)", "Non-para. $p$ (rs)"),
...
apa_table(cbind(alt2_ccatable[,1], papaja::printnum(alt2_ccatable[,2:4], gt1 = FALSE), alt2_ccatable[,5:7], papaja::printp(alt2_ccatable[,8]), papaja::printp(np_rc_f_alt2$np_f), papaja::printp(alt2_ccatable[,9]), papaja::printp(np_rc_f_alt2$np_rc)),
          caption = "Pertinent values from the CCA analysis for each of the five canonical functions created from alternate model two",
          note = "* = <0.05, bold text = both parametric and non-parametric p-value significant. Rc = Canonical Correlation, Rc2 = Canonical Correlation Squared.",
          align = c("l",rep("c", 10) ),
          col.names = c("Dimension",	"Rc",	"Rc2",	"Wilk’s $\\Lambda$",	"Multiple $F$",	"df1",	"df2",	"Para. $p$ ($F$)","Non-para. $p$ ($F$)",	"Para. $p$ (Rc)",	"Non-para. $p$ (Rc)"),
          font_size = "small"
...
row.names(alt2_xcoeftable) <- c("Affective Sharing","Cognitive Empathy","Empathic Concern","Empathic Distress","Silent Films")
alt2_xcoeftable$X <- NULL

row.names(alt2_ycoeftable) <- c("Negative mood/physical symptoms","Negative self-esteem","Ineffectiveness","Interpersonal problems",
                           "Separation anxiety","Social phobia","Physical injury fears","Generalized anxiety","Obsessive compulsive", "Panic/agoraphobia")
...
apa_table(cbind(alt2_xcoeftable[,1], papaja::printnum(alt2_xcoeftable[,2:3], gt1 = FALSE), papaja::printp(alt2_xcoeftable[,4]), papaja::printp(alt2_NPX$x)),
          caption = "Coefficients for X variables, first function.",
          note = "* Survive correction for multiple comparisons (Bonferroni).",
          align = c("l",rep("c", 5) ),
          col.names = c("X Variables", "Standardized X canonical coefficients",	"Structure coefficient (rs)",	"Squared structure",	"Para. $p$ (rs)", "Non-para. $p$ (rs)"),
...
apa_table(cbind(alt2_ycoeftable[,1], papaja::printnum(alt2_ycoeftable[,2:3], gt1 = FALSE),papaja::printp(alt2_ycoeftable[,4]), papaja::printp(alt2_NPY$x)),
          caption = "Coefficients for Y variables, first function.",
          note = "* Survive correction for multiple comparisons (Bonferroni).",
          align = c("l",rep("c", 5) ),
          col.names = c("Y Variables", "Standardized X canonical coefficients",	"Structure coefficient (rs)",	"Squared structure",	"Para. $p$ (rs)", "Non-para. $p$ (rs)"),
